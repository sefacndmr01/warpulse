...
const initMiniMap = async () => {
  ...
  miniMap = L.map(mapEl.value, { center: [20, 10], zoom: 2, zoomControl: true })
  ...
  miniMap.on('click', (e) => {
    ...
    form.value.lat = lat
    form.value.lng = lng
    if (!form.value.location_name)
      form.value.location_name = `${lat.toFixed(4)}, ${lng.toFixed(4)}`
  })
}
          ...
          <div v-else-if="step === 2" class="px-5 py-5 flex flex-col gap-3.5">
            <div>
              <label class="block text-[11px] font-semibold text-muted uppercase tracking-wider mb-1.5">
                Click map to set location <span class="text-red-400 normal-case font-normal tracking-normal">required</span>
              </label>
              <div ref="mapEl" class="w-full h-52 rounded-lg border border-border overflow-hidden" />
              <p class="text-[11px] mt-1.5" :class="form.lat ? 'text-emerald-600' : 'text-muted'">
                {{ form.lat ? `${form.lat.toFixed(4)}, ${form.lng?.toFixed(4)}` : 'No location selected' }}
              </p>
            </div>

            <div>
              <label class="block text-[11px] font-semibold text-muted uppercase tracking-wider mb-1.5">Location Name</label>
              <input
